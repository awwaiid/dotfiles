...
fun! s:scanFunctionFromList(lines)
    let buffile = tempname()
    cal writefile(a:lines,buffile)
    return split(s:system(s:vimbin.'grep-pattern.pl', buffile, '^\s*(?:sub|has|method)\s+(\w+)', '|', 'sort', '|', 'uniq'),"\n")
endf

fun! s:scanFunctionFromSingleClassFile(file)
    return split(s:system(s:vimbin.'grep-pattern.pl', a:file, '^\s*(?:sub|has|method)\s+(\w+)', '|', 'sort', '|', 'uniq'),"\n")
endf
