...
my %sort = (
	...
	'level+refnum'	=> '$b->{data_level} <=> $a->{data_level} ||
				$a->{refnum} <=> $b->{refnum};',
	'level+activity'=>
    # '$b->{data_level} <=> $a->{data_level} || $b->{last_line} <=> $a->{last_line};',
...
sub remake() {
  ...
  @windows = grep { $_->get_active_name ne Irssi::active_win()->get_active_name } @windows;
  unshift @windows, Irssi::active_win();
	...
	foreach my $win (@windows) {
		...
		# filter level
    unless($name eq Irssi::active_win()->get_active_name) {
      ...
      my $channel = ref($server) && $server->channel_find($name);
			my $nick = ref($channel) && $channel->nick_find($server->{nick});
      # !ref($server) && next;
      ...
      if($win->get_active_name eq Irssi::active_win()->get_active_name) {
        $hilight = $show{2};
      }
		}
